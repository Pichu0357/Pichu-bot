...
@Pichu.commands
async def catfact(client, message, search = None):
    if search is None:
        return Embed(f"Here is your cat fact {CAT_WAY:e}", description=choice(CAT_FACTS), color=CAT_FACT_COLOR)

    else:
        matching = [s for s in CAT_FACTS if search in s]

        if matching:
            return Embed(f"Here is your cat fact {CAT_WAY:e}", description=choice(matching), color=CAT_FACT_COLOR)

        else:
            return f"{CAT_SAD:e} sowwy, couldn't find"
